...
API キーは組織を識別する秘密情報です。第三者に知られると不正利用につながるため、次の点を守ってください。

**1. 環境変数に格納して使う**

コードやコマンドにキーを直接書き込まず、環境変数に格納して参照してください。

```bash
export API_KEY="発行された API キー"
...
コミュニティフォーラム、チャット、スクリーンショット、公開リポジトリなど、第三者が閲覧できる場所に API キーを投稿・共有しないでください。

**3. 漏洩した場合**

API キーが漏洩した、または漏洩の疑いがある場合は、直ちに Misora Connect の管理者へご連絡ください。当該キーを即時無効化し、新しいキーを再発行します。
