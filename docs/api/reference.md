...
| パラメータ | 型 | 必須 | デフォルト | 説明 |
|---|---|---|---|---|
| `customer_code` | string | Yes | - | カスタマーコード（9桁の数字） |
| `limit` | integer | No | 100 | 取得件数（最大 1,000） |
| `offset` | integer | No | 0 | 取得開始位置 |
| `order_by` | string | No | `sim_id` | ソートキー。`sim_id` / `customer_code` / `status` / `created_at` / `updated_at` |
...
| ステータス | 条件 |
|---|---|
| `400` | `customer_code` 未指定、`limit` が 1,000 超、不正なソートパラメータ |
| `403` | API キーのカスタマーコード不一致 |
...
| パラメータ | 型 | 必須 | 説明 |
|---|---|---|---|
| `customer_code` | string | Yes | カスタマーコード |
...
| `403 Forbidden` | 認証失敗、またはカスタマーコードの権限不一致 |
